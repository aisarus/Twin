...
const editorialAssets = [...document.querySelectorAll('[data-parallax]')]
const architectureCards = [...document.querySelectorAll('.architecture__card')]
...
function updateScroll() {
  ...
  sceneWash.style.opacity = String(0.12 + warm * 0.52 + smoothstep(0.72, 1, progress) * 0.15)
  document.documentElement.style.setProperty('--story-progress', progress.toFixed(4))
  editorialAssets.forEach(asset => {
    const rectAsset = asset.getBoundingClientRect()
    const center = rectAsset.top + rectAsset.height * 0.5 - innerHeight * 0.5
    const amount = Number(asset.dataset.parallax || 0)
    asset.style.translate = `0 ${(-center * amount).toFixed(1)}px`
  })
  ...
  const activeCard = Math.min(architectureCards.length - 1, Math.max(0, Math.round(local * (architectureCards.length - 1))))
  architectureCards.forEach((card, index) => card.classList.toggle('is-active', index === activeCard))
  raf = 0
}
