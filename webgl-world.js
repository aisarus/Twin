...
float terrainHeight(vec2 xz) {
  ...
  float broad = peak(z, -72.0, 51.0, 3.0) + peak(z, -154.0, 34.0, 4.5);
...
float raymarchTerrain(vec3 ro, vec3 rd, out vec3 hitPos, out float travel) {
  float t = 0.3;
  float maxDistance = mix(118.0, 190.0, float(u_quality) / 2.0);
  int stepLimit = u_quality == 0 ? 32 : (u_quality == 1 ? 50 : 68);
  float hit = 0.0;
  for (int i = 0; i < MAX_STEPS; i++) {
    if (i >= stepLimit) break;
    vec3 p = ro + rd * t;
    float distanceToGround = p.y - terrainHeight(p.xz);
    float epsilon = 0.018 + t * 0.0017;
    if (distanceToGround < epsilon) {
      hit = 1.0;
      hitPos = p;
      travel = t;
      break;
    }
    t += clamp(distanceToGround * 0.56, 0.060, 3.0);
    if (t > maxDistance || p.y > 38.0) break;
  }
  if (hit < 0.5) {
    hitPos = ro + rd * t;
    travel = t;
...
vec3 shadeTerrain(vec3 p, vec3 rd, float travel) {
  ...
  float altitude = saturate((p.y + 6.0) / 20.0);
...
void main() {
  ...
  float lens = mix(1.32, 1.08, smoothstep(0.18, 0.82, u_progress));
  ...
  vec3 hitPos;
  float travel;
  float rawHit = raymarchTerrain(u_cameraPos, rd, hitPos, travel);
  if (rawHit > 0.5) color = mix(color, shadeTerrain(hitPos, rd, travel), terrainReveal);
...
function cameraAt(progress, pointer) {
  let x, y, z, tx, ty, tz
  if (progress < 0.19) {
    const t = progress / 0.19
    x = lerp(0, -0.4, t); y = lerp(0.6, 1.2, t); z = lerp(12, 5, t)
    tx = lerp(0, -0.3, t); ty = lerp(0.1, 0.3, t); tz = lerp(-8, -25, t)
  } else if (progress < 0.54) {
    const t = smoothstep(0, 1, (progress - 0.19) / 0.35)
    x = lerp(-0.4, -3.0, t); y = lerp(1.2, 4.0, t); z = lerp(5, -64, t)
    tx = lerp(-0.3, 0.6, t); ty = lerp(0.3, 4.4, t); tz = lerp(-25, -88, t)
  } else if (progress < 0.85) {
    const t = smoothstep(0, 1, (progress - 0.54) / 0.31)
    x = lerp(-3.0, 4.2, t); y = lerp(4.0, 10.0, t); z = lerp(-64, -128, t)
    tx = lerp(0.6, -0.8, t); ty = lerp(4.4, 10.5, t); tz = lerp(-88, -151, t)
  } else {
    const t = smoothstep(0, 1, (progress - 0.85) / 0.15)
    x = lerp(4.2, 0.0, t); y = lerp(10.0, 16.0, t); z = lerp(-128, -151, t)
    tx = lerp(-0.8, 0.0, t); ty = lerp(10.5, 12.0, t); tz = lerp(-151, -190, t)
  }
  x += pointer.x * 0.5
  y -= pointer.y * 0.24
  tx += pointer.x * 0.14
